...
export default function StopList({ lang, stops, onStopsChange }: StopListProps) {
    ...
    const colors = ['bg-primary-500', 'bg-primary-600', 'bg-indigo-500', 'bg-violet-500', 'bg-purple-500', 'bg-pink-500', 'bg-teal-500', 'bg-cyan-500'];
        ...
        <div>
            ...
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-primary-500"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="8" y1="12" x2="16" y2="12" /><line x1="12" y1="8" x2="12" y2="16" /></svg>
                    <span className="text-sm font-semibold text-surface-700 dark:text-surface-300">{t('stops.title', lang)}</span>
                    <span className="text-xs px-2 py-0.5 rounded-full bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-400 font-bold">{stops.length}</span>
                </div>
                {stops.length > 0 && (
                                ...
                                <Draggable key={stop.id} draggableId={stop.id} index={index}>
                                    {(provided, snapshot) => (
                                        <div
                                            ...
                                            className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 transition-all ${snapshot.isDragging ? 'shadow-lg border-primary-400 scale-[1.02]' : ''}`}
                                        >
                                            <div {...provided.dragHandleProps} className="cursor-grab active:cursor-grabbing text-surface-300 dark:text-surface-600">
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="9" cy="6" r="1.5" /><circle cx="15" cy="6" r="1.5" /><circle cx="9" cy="12" r="1.5" /><circle cx="15" cy="12" r="1.5" /><circle cx="9" cy="18" r="1.5" /><circle cx="15" cy="18" r="1.5" /></svg>
                                            </div>
                                            <div className={`stop-badge ${colors[index % colors.length]}`}>{index + 1}</div>
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm text-surface-800 dark:text-surface-200 truncate">{stop.address}</p>
                                                {!stop.location && (
                                                    <p className="text-xs text-warning flex items-center gap-1 mt-0.5">⚠ {t('location.notFound', lang)}</p>
                                                )}
                                            </div>
                                            {stop.location && (
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="3" className="flex-shrink-0"><polyline points="20 6 9 17 4 12" /></svg>
                                            )}
            ...
            <div className="flex gap-2">
                <div className="flex-1 relative">
                    <input
                        ...
                        className="w-full px-4 py-3 rounded-xl bg-white dark:bg-surface-800 border border-dashed border-surface-300 dark:border-surface-600 text-surface-900 dark:text-surface-100 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/40 transition-all"
                        dir="auto"
                    />
                    {isAdding && (
                ...
                <button
                    ...
                    className="px-4 py-3 rounded-xl gradient-primary text-white font-semibold shadow-lg shadow-primary-500/25 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex-shrink-0"
                >
            ...
            {stops.length === 0 && (
                <p className="text-center text-xs text-surface-400 mt-4 py-3 whitespace-pre-line">{t('stops.emptyMessage', lang)}</p>
            )}
        </div>
    );
}
